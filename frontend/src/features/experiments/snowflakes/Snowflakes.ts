...
class Snowflakes extends InstancedMesh<PlaneGeometry, SpriteNodeMaterial> {
    ...
    uniforms = {
        gravity: uniform(this.params.gravity).setName('gravity'),
        wind: uniform(this.params.wind).setName('wind'),
        friction: uniform(this.params.friction).setName('friction'),
    };
    ...
    constructor({ amount, renderer, viewport }: Parameters) {
        const snowflakeTexture =
            ...
            (() => {
                const texture = new TextureLoader().load('/static/img/snowflake.webp');
        ...
        this.buffers = {
            positions: instancedArray(this.count, 'vec3').setName('positionsBuffer').setPBO(true),
