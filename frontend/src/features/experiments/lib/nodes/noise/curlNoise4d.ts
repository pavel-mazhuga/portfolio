...
export const curlNoise4d = /*#__PURE__*/ Fn(([p_immutable]: [Node<'vec4'>], _builder: NodeBuilder) => {
    const p = vec4(p_immutable).toVar();
    const e = float(0.1);
    const dx = vec4(e, 0.0, 0.0, 1.0).toVar();
    const dy = vec4(0.0, e, 0.0, 1.0).toVar();
    const dz = vec4(0.0, 0.0, e, 1.0).toVar();
    const p_x0 = vec3(snoise3(p.sub(dx).xyz)).toVar();
    const p_x1 = vec3(snoise3(p.add(dx).xyz)).toVar();
    const p_y0 = vec3(snoise3(p.sub(dy).xyz)).toVar();
    const p_y1 = vec3(snoise3(p.add(dy).xyz)).toVar();
    const p_z0 = vec3(snoise3(p.sub(dz).xyz)).toVar();
    const p_z1 = vec3(snoise3(p.add(dz).xyz)).toVar();
