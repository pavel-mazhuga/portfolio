...
const permute_0 = Fn(([x_immutable]: [Node<'vec4'>], _builder: NodeBuilder) => {
    const x = vec4(x_immutable).toVar();

    return mod(x.mul(34.0).add(1.0).mul(x), 289.0);
});

const permute_1 = Fn(([x_immutable]: [Node<'float'>], _builder: NodeBuilder) => {
...
const taylorInvSqrt_0 = Fn(([r_immutable]: [Node<'vec4'>], _builder: NodeBuilder) => {
    const r = vec4(r_immutable).toVar();

    return sub(1.79284291400159, mul(0.85373472095314, r));
});

const taylorInvSqrt_1 = Fn(([r_immutable]: [Node<'float'>], _builder: NodeBuilder) => {
...
const grad4 = Fn(([j_immutable, ip_immutable]: [Node<'float'>, Node<'vec4'>], _builder: NodeBuilder) => {
    ...
    s.x.assign(select(p.x.lessThan(float(0)), float(1), float(0)));
    s.y.assign(select(p.y.lessThan(float(0)), float(1), float(0)));
    s.z.assign(select(p.z.lessThan(float(0)), float(1), float(0)));
    s.w.assign(select(p.w.lessThan(float(0)), float(1), float(0)));
...
const simplexNoise4d = Fn(([v_immutable]: [Node<'vec4'>], _builder: NodeBuilder) => {
    ...
    const i0 = vec4().toVar();
    const isX = vec3((step as (a: unknown, b: unknown) => ReturnType<typeof step>)(x0.yzw, x0.xxx)).toVar();
    const isYZ = vec3((step as (a: unknown, b: unknown) => ReturnType<typeof step>)(x0.zww, x0.yyz)).toVar();
    ...
    const j0 = float(
        // @ts-ignore
        (permute as (x: unknown) => Node<'vec4'>)(
            (permute as (x: unknown) => Node<'vec4'>)(
                (permute as (x: unknown) => Node<'vec4'>)((permute as (x: unknown) => Node<'vec4'>)(i.w).add(i.z)).add(
                    i.y,
                ),
            ).add(i.x),
        ),
