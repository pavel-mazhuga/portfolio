...
class Demo extends BaseExperience {
    ...
    particlesBasePositionsBuffer?: StorageBufferNode<'vec3'>;
    ...
    strengthBuffer?: StorageBufferNode<'float'>;
    ...
    constructor(canvas: HTMLCanvasElement) {
        ...
        gltfLoader.load('/static/gltf/face2.glb', (gltf) => {
            gltf.scene.traverse((node) => {
                if (node instanceof Mesh) {
                    ...
                    this.strengthBuffer = storage(
                        new StorageInstancedBufferAttribute(this.amount, 1),
                        'float',
                        this.amount,
                    );

                    const strengthBuf = this.strengthBuffer;
                    ...
                    const initParticlesCompute = Fn(() => {
                        ...
                        strengthBuf.element(instanceIndex).assign(0);
                    })().compute(this.amount);
                    ...
                    this.updateParticlesCompute = Fn(() => {
                        ...
                        const strength = strengthBuf.element(instanceIndex);
        ...
        this.postProcessing.outputNode = add(
            scenePassColor as unknown as Node<'vec4'>,
            bloomPass as unknown as Node<'vec4'>,
        );
    ...
    async render() {
        ...
        this.delta = elapsed - this.prevTime;
        this.prevTime = elapsed;

        this.controls?.update();
        this.pointerHandler.update();

        if (this.updateParticlesCompute) {
