...
import { curlNoise4d } from '@/features/experiments/lib/nodes/noise/curlNoise4d';
import { rotate3dY } from '@/features/experiments/lib/nodes/rotate-3d-y';
...
class ParticlesMesh extends InstancedMesh<PlaneGeometry, SpriteNodeMaterial> {
    ...
    buffers: {
        basePositions?: StorageBufferNode<'vec3'>;
        colors?: StorageBufferNode<'vec3'>;
    ...
    constructor(
        ...
        this.updateCompute = Fn(() => {
            ...
            const offset = 2.5;
            const morphIndex = select(
                this.uniforms.activeIndex.equal(0),
                instanceIndex,
                instanceIndex.add(this.amount),
            );
            const basePosition = buffer.element(morphIndex).toVar();
