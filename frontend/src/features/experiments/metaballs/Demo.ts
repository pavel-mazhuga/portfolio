...
import { createFresnelNode } from '../lib/nodes/lighting/fresnel';
import { opSmoothUnion } from '../lib/sdf/math';
import { sdSphere } from '../lib/sdf/shapes';
...
class Metaballs extends BaseExperience {
    ...
    uniforms: {
        uK: UniformNode<'float', number>;
        uSpeed: UniformNode<'float', number>;
        uColorA: UniformNode<'color', Color>;
        uColorB: UniformNode<'color', Color>;
        uCameraDist: UniformNode<'float', number>;
    };
    ...
    constructor(canvas: HTMLCanvasElement) {
        ...
        const sceneSDF = Fn(([p]: [Node<'vec3'>]) => {
            const t = time.mul(uSpeed);
            let finalDist: Node<'float'> | null = null;
        ...
        const calcNormal = Fn(([p]: [Node<'vec3'>]) => {
        ...
        const phongLight = Fn(([ro, hitPos, baseColor]: [Node<'vec3'>, Node<'vec3'>, Node<'vec3'>]) => {
