/** Inlined from glsl-utils/noise/curl-noise.glsl (glslify export pragma stripped). */
export const CURL_NOISE_GLSL = /* glsl */ `
vec3 mod289_curl(vec3 x) {
    return x - floor(x * (1.0 / 289.0)) * 289.0;
}

vec4 mod289_curl(vec4 x) {
    return x - floor(x * (1.0 / 289.0)) * 289.0;
}

vec4 permute_curl(vec4 x) {
    return mod289_curl(((x * 34.0) + 1.0) * x);
}
...
vec3 snoiseVec3_curl(vec3 x) {
    float s = snoise_curl(vec3(x));
    float s1 = snoise_curl(vec3(x.y - 19.1, x.z + 33.4, x.x + 47.2));
    float s2 = snoise_curl(vec3(x.z + 74.2, x.x - 124.5, x.y + 99.4));
    vec3 c = vec3(s, s1, s2);
    return c;
}
...
vec3 curlNoise(vec3 p) {
    ...
    vec3 p_x0 = snoiseVec3_curl(p - dx);
    vec3 p_x1 = snoiseVec3_curl(p + dx);
    vec3 p_y0 = snoiseVec3_curl(p - dy);
    vec3 p_y1 = snoiseVec3_curl(p + dy);
    vec3 p_z0 = snoiseVec3_curl(p - dz);
    vec3 p_z1 = snoiseVec3_curl(p + dz);
